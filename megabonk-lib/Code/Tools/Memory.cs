...
internal class UMemory : UShared
{
    ...
    internal static ulong ScanSpecific(Process process, string signature, ulong startAddress, int size)
    {
        byte[] readBytes = ReadMemoryBytes(process, startAddress, size);
        if (readBytes.Length != 0)
        {
            string mask = USignature.GetMask(signature);
            byte[] search = USignature.GetBytes(signature);

            int offset = FindInArray(readBytes, search, mask);
            if (offset != -1) return startAddress + (ulong)offset;
        }
    ...
    internal static ulong ScanSingle(Process process, string signature)
    {
        ...
        List<ulong[]> readSections = GetReadableSections(process, peHeader);
    ...
    internal static List<ulong[]> GetReadableSections(Process process, byte[] peHeader)
    {
